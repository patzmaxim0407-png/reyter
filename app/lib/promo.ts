...
export function promoCheck(
  ...
  /* Округлення вниз, а не до найближчого, — і це НЕ те, що робив
     старий сайт. Правила бази звіряють знижку з `subtotal * value
     / 100`, порахованим цілочисельно, тобто з відкиданням дробу.
     Math.round давав на копійку більше, правило відхиляло весь
     запис, і замовлення мовчки не створювалось: наприклад 10%
     від 465 грн — клієнт рахував 47, правило дозволяло 46.
     Втрата для покупця — менша за гривню; альтернатива —
     втрачене замовлення. */
  let discount = promo.type === 'fixed'
    ? Math.min(Number(promo.value) || 0, eligibleTotal)
    : Math.floor(eligibleTotal * (Number(promo.value) || 0) / 100);
