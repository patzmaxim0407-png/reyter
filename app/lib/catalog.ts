...
import type { Availability, Category, Color, NoteId, Product, Stock } from './types';
...
export type Catalogue = {
  ...
  categories?: Category[];
  /** Поріг безкоштовної доставки по Україні, грн.
   *
   *  Їде разом із каталогом, а не читається окремо: його треба і
   *  в кошику, і на сторінці товару, і в накладній — тобто там,
   *  де каталог уже є. Порожньо — беремо число з коду. */
  freeFrom?: number;
  /** Як поріг мінявся — найновіше згори. Потрібен лише адмінці:
   *  замовлення, оформлені до появи заморожування, свого числа не
   *  мають, і єдиний спосіб показати їхнє правило чесно — знайти
   *  його в історії за датою. */
  freeLog?: { at: string; value: number }[];
};

/** Який поріг діяв у цю мить.
 *
 *  Порядок відповідей: запис історії, що покриває цю дату →
 *  типове число з коду. Теперішній поріг сюди НЕ підставляємо:
 *  замовлення від чотирнадцятого числа не могло знати про
 *  сімнадцяте, і показати сьогоднішнє число як його правило
 *  означало б збрехати рівно там, де людина шукає правду.
 *
 *  Історії немає зовсім — тоді теперішній: іншого джерела просто
 *  не існує. */
export function freeFromAt(
  log: { at: string; value: number }[] | undefined,
  when: Date,
  now: number
): number {
  if (!log?.length) return now;
  const t = when.getTime();
  if (!Number.isFinite(t) || t <= 0) return now;

  const hit = [...log]
    .sort((a, b) => Date.parse(b.at || '') - Date.parse(a.at || ''))
    .find((x) => Date.parse(x.at || '') <= t);

  if (!hit) return FREE_DELIVERY_FROM;
  return hit.value > 0 ? hit.value : FREE_DELIVERY_FROM;
}

/** Три примітки під кнопкою «Додати в кошик». Один перелік на
 *  сайт і на адмінку: інакше вимикач у картці й картка на сайті
 *  розійшлись би порядком або складом. */
export const NOTES: { id: NoteId; key: string; title: string }[] = [
  { id: 'np', key: 'p.note1', title: 'Нова Пошта — відправимо сьогодні' },
  { id: 'free', key: 'p.note2', title: 'Безкоштовна доставка від порога' },
  { id: 'intl', key: 'p.note3', title: 'Міжнародна доставка' }
];

/** Підставити поріг у текст.
 *
 *  Число живе в одному місці, а згадується в багатьох: рядок, що
 *  біжить угорі, обіцянка на головній, примітка в картці товару.
 *  Доти воно було вписане в кожен із них окремо — і змінивши
 *  налаштування, магазин обіцяв би трьома різними числами
 *  водночас. */
export function withFree(text: string, from: number): string {
  return String(text || '').replaceAll('{free}', String(from));
}

/** Скільки треба набрати білизни для безкоштовної доставки.
 *
 *  Одне місце на весь проєкт: сайт, кошик, картка товару й
 *  адмінка мусять називати те саме число. Розійшовшись, вони
 *  пообіцяли б покупцеві одне, а порахували інше. */
export function freeFromOf(c?: { freeFrom?: number } | null): number {
  const n = Math.round(Number(c?.freeFrom) || 0);
  return n > 0 ? n : FREE_DELIVERY_FROM;
}
