...
/** Розміри, які беруть найчастіше. Немає їх — товар у видачі
 *  наявний, а купити нема чого; немає крайніх — втрата менша. */
const CORE = ['S', 'M', 'L'];
...
export function tipsFor(row: Row, ctx: Context): Tip[] {
  ...
  /* Розміри. Порада має сенс лише тоді, коли товар справді
     беруть і бракує саме ходових: «немає XS» у речі з одним
     продажем — не проблема, а шум, який витісняє важливе. */
  if (solid && ctx.gone.length && ctx.sizes > 0 && ctx.gone.length < ctx.sizes) {
    const core = ctx.gone.filter((s) => CORE.includes(s));
    if (core.length) {
      tips.push({
        kind: 'sizes',
        urgency: core.length >= 2 ? 2 : 1,
        title: `Немає ходових розмірів: ${core.join(', ')}`,
        what:
          `лишилось ${ctx.sizes - ctx.gone.length} з ${ctx.sizes}` +
          ` — саме ${core.join(' і ')} беруть найчастіше`,
        todo:
        ...
        money: perUnit ? Math.round(pace * 20 * perUnit) : 0
      });
    }
  }
