...
export interface Band {
  id: BandId;
  /* Емодзі лишились у моделі задля сумісності, але їх більше не
     малюють: набір був різномастий — монохромні гліфи поруч із
     кольоровими картинками, — і в кожній смузі найяскравішим було
     саме емодзі, хоч означало те, що вже сказано словом. */
  icon: string;
  title: string;
...
export function orderMismatch(o: AdminOrder): string {
  ...
  /* Замовлення, оформлені до 14.08.2026, тримали доставку ПОЗА
     сумою: total дорівнював «товари − знижка», а ціна доставки
     лежала поруч окремим полем. Це стара форма запису, а не
     зіпсоване замовлення, і сварка на неї була б рівно тією
     фальшивою тривогою, від якої ця перевірка й лікується.

     Приймаємо обидві форми замість того, щоб різати за датою:
     дата коміту — це не дата викладки, і помилившись на кілька
     годин, я нагенерував би саме те, чого уникаю. Ціна цього —
     підроблений підсумок, що випадково збігся зі старою формою,
     пройде повз. Але така підробка мусила б рівно дорівнювати
     сумі без доставки, тобто це не діра, а межа роздільності. */
  const legacy = Math.max(0, sub - off);
  const fits = Math.abs(want - tot) <= 1 || (ship > 0 && Math.abs(legacy - tot) <= 1);
...
export function freeShipOf(order: AdminOrder, c?: Catalogue | null): FreeShip {
  ...
  /* Поріг, ЗАМОРОЖЕНИЙ у мить оформлення, — і лише коли його там
     немає, беремо теперішній із налаштувань.

     Магазин міняє поріг, коли вважає за потрібне; замовлення від
     цього змінюватись не має. Покупець оформив його за правилом,
     яке бачив на сайті того дня, — і саме це правило лишається в
     силі. Так само, як собівартість фіксується в мить продажу, а
     нова партія міняє майбутнє, а не минуле.
