...
/** Зразок листа — рівно той, що піде покупцеві.
 *
 *  Збирає його воркер, тим самим кодом, що й надсилає. Другого
 *  шаблону в адмінці немає навмисно: два шаблони розійшлись би
 *  тихо, і зразок почав би показувати не те, що надходить людям.
 *  Помітили б це вже з отриманого листа — тобто пізно.
 *
 *  sample — чиє імʼя підставити замість змінної Resend, щоб зразок
 *  читався як лист, а не як шаблон. */
export function previewLetter(cab: Cabinet, letter: Letter, sample = '') {
  return ask<{ html?: string }>(cab, { type: 'mk-preview', sample, ...letter });
}
