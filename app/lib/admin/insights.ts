/* ============================================================
   ...
   ЗВІДКИ БЕРЕТЬСЯ МАРЖА. З собівартості товару, яку власник
   вписує в картці. Її немає в жодному опублікованому документі й
   не буде: у каталозі, який читає весь світ, ціні закупівлі не
   місце. Товарам без собівартості маржа не вигадується — вони
   рахуються окремо, і екран про це каже прямо. Порахована
   «приблизна маржа» гірша за відсутню: за нею ухвалять рішення.
...
export function linesOf(o: AdminOrder, byId: Map<string, Product>): SoldLine[] {
  ...
  return items.map((i) => {
    ...
    const cost = p && Number.isFinite(Number(p.cost)) && Number(p.cost) > 0 ? Number(p.cost) : null;
    return {
      id: String(i.id || ''),
