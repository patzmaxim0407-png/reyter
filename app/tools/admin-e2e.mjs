...
/* ---------- Магазин лишився цілим ----------
   Пропускаємо, якщо магазин на іншому домені: підняти обидва
   хости в одному локальному запуску не можна, і перевірка
   падала б на порожньому місці. */

if (SHOP === BASE) {
  console.log('· магазин на іншому домені — перевіряємо його окремим прогоном');
} else {
await go(SHOP + '/');
