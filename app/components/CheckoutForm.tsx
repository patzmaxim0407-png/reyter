  ...
  async function submit() {
    ...
    try {
      ...
      const customer: Customer = {
        ...
        /* Поріг безкоштовної доставки ЗАМОРОЖУЄМО в замовленні.
           Магазин змінить його завтра — а це замовлення має й
           через рік показувати правило, за яким його оформили.
           Так само, як собівартість фіксується в мить продажу.

           Усередині customer, а не окремим полем: правила бази
           перелічують дозволені поля замовлення й публікуються
           руками, тож нове поле верхнього рівня спинило б живий
           сайт до тієї хвилини — і спинило б мовчки. */
        freeFrom: freeFromOf(c)
      };
