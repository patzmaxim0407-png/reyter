...
export default function ManualOrder({
  ...
  /* Замовлення з накладною в частині «куди» і «що» вважаємо
     закритим: накладна вже надрукована, товар списаний і лежить
     у коробці. Решту — імʼя, телефон, пошту, коментар, суми —
     правити можна, це буває треба.

     «Підготовка до відправлення» тут разом із «Відправлено» не
     випадково: накладну друкують саме в цю мить, і виправлена
     після неї адреса нікуди вже не поїде. */
  const sealed =
    !!order &&
    (order.status === 'packing' || order.status === 'shipped' || order.status === 'done');
            ...
            <p className="ao-note ao-note--locked">
              ...
              {order?.status === 'done'
                ...
                : order?.status === 'packing'
                  ? 'з накладною'
                  : 'відправлене'}{' '}
              — адресу й товари змінити не можна. Помилка в адресі виправляється новою накладною, помилка в товарах —
