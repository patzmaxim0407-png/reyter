      ...
      {open ? (
        <div className="ao-card__details">
          ...
          <div className="ao-sums">
            ...
            {/* А коли порога не набрали — доставку платить
                отримувач у відділенні. Це теж мало бути видно:
                рядок «Доставка» просто зникав, і з картки не
                зрозуміло, чи ми про неї забули, чи так і треба.

                Поруч — скільки бракує до безкоштовної: менеджер
                говорить із людиною по телефону саме зараз, і це
                єдина мить, коли ця цифра ще може щось змінити. */}
            {!ship && !free.reached && !o.pickup && !isIntl ? (
              <div className="ao-sumline is-onhim">
                <span>Доставка</span>
                <span>
                  за тарифом перевізника — платить отримувач
                  {free.need > 0 ? <em> · до безкоштовної бракує {fmt(free.need)} грн</em> : null}
                </span>
              </div>
            ) : null}
            {/* Рахунок має сходитись на екрані. Беремо суму з
                ...
                входила, і картка показувала «2190 + 540 = 2190». */}
          ...
          <div className="ao-ttn__acts">
            ...
            {/* За кордон накладну створює не наш кабінет: це інша
                система Нової Пошти, з іншим договором і власним
                входом. Кнопки тут не буде — але мовчазна порожнеча
                читалась як поломка, тож кажемо прямо, що робити. */}
            {isIntl && !hasTtn && st !== 'cancelled' ? (
              <p className="ao-note ao-note--intl">
                Міжнародну накладну створюють у кабінеті{' '}
                <a href="https://novapost.com" target="_blank" rel="noreferrer">
                  Nova Post
                </a>{' '}
                — наш договір із Новою Поштою на неї не діє. Номер звідти впишіть у поле вище:
                далі все працює як завжди — і лист покупцеві, і відстеження.
              </p>
            ) : null}
