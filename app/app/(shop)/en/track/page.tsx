...
export default async function TrackPageEn({ searchParams }: { searchParams: Promise<{ num?: string }> }) {
  ...
  return <div className="container account-page"><PageTop /><h1 className="section-title">Track your order</h1><p className="account-note">Enter your order number and the phone used at checkout to see its current status.</p><TrackForm divider={false} initialNum={num ?? ''} /></div>;
}
