...
(function () {
  ...
  /* ---------- Списання використання ----------
     Лічильник збільшує браузер після створення замовлення.
     Правила бази дозволяють рівно +1 і жодних інших полів,
     тож обнулити код чи «розширити» ліміт так не вийде.
     Без цього ліміт не діяв би взагалі: усі перевірки
     спираються саме на usedCount. */

  R.promoConsume = async function (code) {
    const id = R.promoNormalize(code);
    if (!id || !R.fb || !R.fb.enabled) return false;
    try {
      await R.fb.db.collection('promos').doc(id).update({
        usedCount: firebase.firestore.FieldValue.increment(1)
      });
      return true;
    } catch (e) {
      // не вдалося — замовлення вже створене, ламати його через
      // лічильник не можна; адмінка все одно рахує по замовленнях
      return false;
    }
  };
